...
def export_trees_to_fc_CCRS(trees,response,regressors,regressorsGEECollectionName,regressorsGEENames,regressorsGEEScaling2, \
...
def export_list_to_fc(list, listName, asset_id, description="geemap_rf_export"):
    """Function that creates a feature collection with a property tree which contains the string representation of decision trees and exports to ee asset for later use
    ...
    # encode return values (\n) as #, use to parse later
    features = [
        ee.Feature(null_island, {listName: list}) for tree in trees
    ]
    # cast as feature collection
    fc = ee.FeatureCollection(features)
